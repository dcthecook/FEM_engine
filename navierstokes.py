...
class NavierStokes2D(moderngl_window.WindowConfig):
    ...
    resource_dir = (Path(__file__) / "C:/Users/Enea/FEM/resources").absolute()
    ...
    def apply_left_stream(self):
        # Apply a constant velocity (external flow) at the left boundary of the simulation
        # You can adjust this logic to apply the boundary condition in a specific region
        for y in range(self.wnd.buffer_height):
            pos = (0, y)  # Left side of the domain
            self.walls_fbo.use()
            self.drop_prog["pos"].value = pos
            self.drop_prog["force"].value = 2.0  # Apply a force to the fluid
    ...
    def render(self, time, frame_time):
        # Apply the left-side stream at each frame
        self.apply_left_stream()
